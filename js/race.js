...
const Race = {
  ...
  init(circuitId, weather = 'dry', playerStrategies = {}) {
    ...
    const activeDrivers = F1Data.drivers.filter(d => d.teamId && d.teamId !== 'free_agent' && !d.retired);
  ...
  simulateLap() {
    ...
    if (typeof currentHumidity !== 'undefined') {
      ...
      if (prevWeather !== s.weather) {
        if (s.weather === 'heavy_rain')
          lapEvents.push({ lap, type:'weather', message:S('race.heavy_rain') });
        else if (s.weather === 'light_rain')
          lapEvents.push({ lap, type:'weather', message:S('race.rain_start') });
        else
          lapEvents.push({ lap, type:'weather', message:S('race.track_dry') });
      }
    }
    ...
    s.grid.forEach(car => {
      ...
      if (dnf) {
        ...
        lapEvents.push({
          lap,
          type:    'dnf',
          message: `❌ ${car.driver.firstName} ${car.driver.name} — Abandon (${dnf.reason === 'crash' ? 'Accident' : S('race.mech_issue')}) Tour ${lap}`,
        });
        return;
      }
      ...
      if (pitDecision.pit && lap < s.totalLaps - 2) {
        ...
        const pitReasonMessages = {
          planned: S('race.planned_stop'),
          undercut: 'on couvre l’undercut d’un rival',
          safety_car_opportunity: S('race.sc_opp'),
          weather_change: S('race.weather_stop'),
          tyre_dead: S('race.crit_tyres'),
          team_order: S('race.wall_order'),
        };
        const pitReasonText = pitReasonMessages[pitDecision.reason] || S('race.strat_adj');
      ...
      for (let i = 1; i < racing.length; i++) {
        ...
        if (overtook) {
          ...
          lapEvents.push({
            lap,
            type: 'overtake',
            message: `🏎️ ${attacker.driver.firstName} ${attacker.driver.name} dépasse ${defender.driver.firstName} ${defender.driver.name}${cir.overtakingDifficulty > 0.7 ? S('race.overtake') : ''}`,
          });
        }
      }
    }
    ...
    if (scRoll.active && !s.safetyCar.active) {
      ...
      lapEvents.push({ lap, type: 'safety_car', message: S('race.sc_deployed') });
    }
    ...
    if (lap >= s.totalLaps) {
      s.finished = true;
      lapEvents.push({ lap, type: 'finish', message: S('race.chequered') });
    }
  ...
  setDriverMode(driverId, mode) {
    ...
    this.state.events.push({ lap: this.state.currentLap, type: 'team_order', message: `📻 ${car.driver.firstName} ${car.driver.name} reçoit l'ordre : ${car.orderMode === 'attack' ? 'attaque' : car.orderMode === 'save' ? S('race.tyre_saving') : 'rythme normal'}` });
    return true;
  },
